...
def registrar_mascota():
    ...
    precio = float(input("Precio de atencion de la mascota: "))
...
def registrar_venta(clientes, inventario):
    ...
    while True:
        nombre_productos = input("Nombre del producto (deje vacio para finalizar): ")
        if not nombre_productos:
            break 
        producto = next((p for p in inventario.lista_de_productos if p.nombre == nombre_productos), None)
        if producto:
...
def mostrar_menu():
    ...
    print("8.Registrar alerta de inventario")
    print("9.Salir")
